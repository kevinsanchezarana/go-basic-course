...
func readCompleteFile() {
	...
	// Use a valid path!
	file, err := os.Open("reading_file/test.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
	...
	file, err = os.Open("reading_file/main.go")
	if err != nil {
		log.Fatal(err)
	}
	...
	data, err = ioutil.ReadFile("reading_file/test.txt")
	if err != nil {
		log.Fatal(err)
	}
	log.Printf("Data read: %s\n", data)
...
func readFileByLines() {
	// opening the file in read-only mode. The file must exist (in the current working directory)
	// use a valid path!
	file, err := os.Open("reading_file/my_file.txt")
	// error handling
	if err != nil {
		log.Fatal(err)
	}
